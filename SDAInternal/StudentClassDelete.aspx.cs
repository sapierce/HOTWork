...
{
    public partial class StudentClassDelete : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                if ((Request.QueryString["CID"] != null) && (Request.QueryString["ID"] != null))
                {
                    ...
                    if ((!String.IsNullOrEmpty(Request.QueryString["CID"])) && (!String.IsNullOrEmpty(Request.QueryString["ID"])))
                    {
                        ...
                        bool response = methodsClass.DeleteStudentCourse(Convert.ToInt32(Request.QueryString["CID"].ToString()), Convert.ToInt32(Request.QueryString["ID"].ToString()));

                        // Was the delete successful?
                        if (!response)
                        {
                            // Send the error and output the error message
                            functionsClass.SendErrorMail("StudentClassDelete", new Exception(response.ToString()), Request.QueryString["CID"].ToString());
            ...
            catch (Exception ex)
            {
                ...
                functionsClass.SendErrorMail("StudentClassDelete: PageLoad", ex, "");
